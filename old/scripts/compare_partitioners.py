...
logger = logging.getLogger("test partition")
...
def compare_partitioners(model, sample):
	...
	print("Proportion of nodes profiled :")
	print(
		f"\tMem - {(n_profiled_mem / len(trace.graph.nodes)) * 100:.2f}% (Non-Zero : {(n_profiled_mem_nz / len(trace.graph.nodes)) * 100:.2f}%)"
	)
	print(
		f"\tTime - {(n_profiled_t / len(trace.graph.nodes)) * 100:.2f}% (Non-Zero: {(n_profiled_t_nz / len(trace.graph.nodes)) * 100:.2f}%)"
	)
	...
	for name, partitioner in zip(
		...
		print(
			f'\tMemory sent : {["%.1fMB" % m for m in memories_used]} (total = {sum(memories_used[:-1]):.1f}MB)\n'
		)
