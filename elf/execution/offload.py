...
class PinnedHostTensorPool:
    ...
    def allocate(self, required_elems: int, dtype: torch.dtype) -> torch.Tensor:
        # First-fit from existing parents of this dtype
        ...
        for parent in self._parents_by_dtype.get(dtype, []):
            ...
            for i, (off, length) in enumerate(free_list):
                if length >= request_elems:
                    # Allocate from start of the segment
                    alloc_off = off
        ...
        # No space available: create a new parent chunk and allocate from it
        parent_elems = self._bucket_size(request_elems)
        ...
        # Allocate from this new parent
        alloc_off = 0
        remaining = parent_elems - request_elems
        if remaining == 0:
            self._free_segments[id(parent)] = []
        else:
            self._free_segments[id(parent)] = [(request_elems, remaining)]
        view = torch.as_strided(parent, size=(request_elems,), stride=(1,), storage_offset=alloc_off)
        self._view_to_segment[id(view)] = (parent, alloc_off, request_elems)
        return view
    ...
    def reserve(self, required_bytes: int, dtype: torch.dtype = torch.float32) -> None:
        ...
        elems = self._bucket_size(elems_exact)
        parent = torch.empty(elems, dtype=dtype, device=self.device, pin_memory=True)
        self._parents_by_dtype.setdefault(dtype, []).append(parent)
        self._free_segments[id(parent)] = [(0, elems)]
...
class OffloadToCPU:
    ...
    def __init__(self, target_device: torch.device | str = "cpu", pool: Optional["PinnedHostTensorPool"] = None) -> None:
    ...
    def _save_hook(self, tensor: torch.Tensor) -> Dict[str, Any]:
        ...
        need_copy = (
            storage_ptr not in self._storage_map
            or required_elems > self._storage_len.get(storage_ptr, 0)
        )
        if need_copy:
            # Create a flat view that covers [0:required_elems) with unit stride
            full_view = torch.as_strided(
                tensor.detach(),
                size=(required_elems,),
                stride=(1,),
                storage_offset=0,
            ...
            # Allocate a new CPU tensor (prefer pooled pinned memory)
            use_pinned = (
                torch.cuda.is_available() and self.target_device.type == "cpu"
            )
            if use_pinned and self._pool is not None:
                cpu_storage = self._pool.allocate(required_elems, tensor.dtype)
            else:
            ...
            if self._offload_stream is not None and tensor.is_cuda:
                # Ensure offload stream follows current stream
                self._offload_stream.wait_stream(torch.cuda.current_stream())
                with torch.cuda.stream(self._offload_stream):
                    cpu_storage.copy_(full_view, non_blocking=True)
                    ev = torch.cuda.Event()
                    ev.record(self._offload_stream)
                    self._offload_events[storage_ptr] = ev
            else:
                cpu_storage.copy_(full_view, non_blocking=False)
    ...
    def _restore_hook(self, payload: Dict[str, Any]) -> torch.Tensor:
        ...
        if storage_ptr not in self._gpu_cache:
            self._gpu_cache[storage_ptr] = base_cpu.to(
                payload["device"], non_blocking=False
            )
    ...
    def prefetch(self) -> None:
        ...
        for storage_ptr, base_cpu in list(self._storage_map.items()):
            ...
            # Make prefetch stream wait for the D2H offload to complete, if any
            off_ev = self._offload_events.get(storage_ptr)
            if off_ev is not None:
                self._prefetch_stream.wait_event(off_ev)

            self._prefetch_stream.wait_stream(torch.cuda.current_stream()) # wait for current stream to finish (so that the prefetching starts when we want, and not before!)
    ...
    # Public API to release pooled memory and internal maps once this offloader
    # is no longer needed (typically after the corresponding backward finished).
