...
class ReversibleColumnNetwork(nn.Module):
    def __init__(self, in_channels, out_channels, num_blocks, kernel_size=3, padding=1):
        ...
        self.blocks = nn.ModuleList([ReversibleColumnBlock(out_channels, kernel_size, padding) for _ in range(num_blocks)])
...
class RCN(nn.Module):
    def __init__(self, in_channels, hidden_channels, num_blocks, num_columns):
        ...
        self.columns = nn.ModuleList([ReversibleColumnNetwork(in_channels, hidden_channels, num_blocks) for _ in range(num_columns)])
