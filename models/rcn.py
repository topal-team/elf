...
class LayerNorm(nn.Module):
	...
	def __init__(
		self, normalized_shape, eps=1e-6, data_format="channels_first", elementwise_affine=True
	):
		super().__init__()
...
class Decoder(nn.Module):
	def __init__(
		...
		self.projback = nn.Sequential(
			nn.Conv2d(in_channels=dim[-1], out_channels=4**2 * 3, kernel_size=1), nn.PixelShuffle(4)
		)
	...
	def _build_decode_layer(self, dim, depth, kernel_size):
		...
		for i in range(1, len(dim)):
			...
			proj_layers.append(
				nn.Sequential(nn.Conv2d(dim[i - 1], dim[i], 1, 1), norm_layer(dim[i]), nn.GELU())
			)
...
class SubNet(nn.Module):
	def __init__(self, channels, layers, kernel_size, first_col, dp_rates, save_memory) -> None:
		...
		self.alpha0 = (
			nn.Parameter(
				shortcut_scale_init_value * torch.ones((1, channels[0], 1, 1)), requires_grad=True
			)
		...
		self.alpha1 = (
			nn.Parameter(
				shortcut_scale_init_value * torch.ones((1, channels[1], 1, 1)), requires_grad=True
			)
			if shortcut_scale_init_value > 0
			else None
		)
		self.alpha2 = (
			nn.Parameter(
				shortcut_scale_init_value * torch.ones((1, channels[2], 1, 1)), requires_grad=True
			)
			if shortcut_scale_init_value > 0
			else None
		)
		self.alpha3 = (
			nn.Parameter(
				shortcut_scale_init_value * torch.ones((1, channels[3], 1, 1)), requires_grad=True
			)
			if shortcut_scale_init_value > 0
			else None
		)
		...
		if not inter_supv:
			...
			if num_classes <= 1000:
				...
				self.decoder_blocks = nn.ModuleList(
					[
						Decoder(depth=[1, 1, 1, 1], dim=channels, block_type=ConvNextBlock, kernel_size=3)
						for _ in range(3)
					]
				)
			else:
...
def revcol_base(
	save_memory, inter_supv=True, drop_path=0.4, num_classes=1000, kernel_size=3, head_init_scale=None
):
...
def revcol_large(
	save_memory, inter_supv=True, drop_path=0.5, num_classes=1000, kernel_size=3, head_init_scale=None
):
...
def get_gpu_device(*args):
	fwd_gpu_devices = list(
		set(arg.get_device() for arg in args if isinstance(arg, torch.Tensor) and arg.is_cuda)
	)
