...
class MLP(nn.Module):
	def __init__(self, in_channels, out_channels, mid_channels):
		...
		self.mlp_block = nn.Sequential(self.mlp1, self.gelu, self.mlp2)
...
class FNO1d(nn.Sequential):
	def __init__(self, modes, width, block_number=4):
		...
		for i in range(0, block_number - 1):
			self.add_module(
				"kernel_layer_" + str(i),
				integral_kernel_block(self.width, self.modes1, self.remote_conv, is_gelu=True),
			)
		self.add_module(
			"kernel_layer_" + str(block_number - 1),
			integral_kernel_block(self.width, self.modes1, self.remote_conv, is_gelu=False),
		)
	...
	def is_spectral_weights_equal(self):
		print(
			"compare data ptr ",
			self.kernel_layer_0.conv.einsum.weights1.data_ptr()
			== self.kernel_layer_1.conv.einsum.weights1.data_ptr(),
		)
		return torch.all(
			(self.kernel_layer_0.remote_conv.weights1 - self.kernel_layer_1.remote_conv.weights1) == 0
		)
...
class ModelParallelFNO1d(nn.Module):
	def __init__(self, modes, width, devices):
		...
		self.seq2 = nn.Sequential(self.projection).cuda(self.devices[1])
