...
from elf.partitioners import partition
...
def test_partition(mode, model_config):
	...
	def check_partition(model, n, sample, mode):
		result = partition(copy.deepcopy(model), n, sample, mode, TRACERS["fx"])
		parts, signatures = result.create_blocks(), result.signatures
...
def test_partition_single_stage():
	...
	result = partition(copy.deepcopy(model), 1, sample, PARTITIONERS["naive"], TRACERS["fx"])

	assert result.n == 1
	assert len(result.signatures) == 1

	# Single stage should produce same output as original
	blocks = result.create_blocks()
	output_original = model(sample)
	output_partitioned = blocks[0](*[sample])[0]
