...
rule solve_method:
	...
	shell:
		"python ../pipeline-ilps/runall.py --config {input} --output {output} --method {wildcards.method} --processors {params.ngpus} --time-limit 600 --scheduler {params.scheduler} --mem {params.memgpu} --nmb {params.nmb} 1> {log}.out 2> {log}.err"
