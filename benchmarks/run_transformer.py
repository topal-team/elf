		...
		if self.need_master_weights:
			...
			for model_param, master_param in self.param_mapping.items():
				if model_param.grad is not None:
					...
					master_param.grad.copy_(model_param.grad, non_blocking=True)

			torch.cuda.synchronize()  # make sure all copies are completed
			...
			for model_param, master_param in self.param_mapping.items():
				model_param.data.copy_(master_param.data, non_blocking=True)
...
def parse_args():
	...
	parser.add_argument("--memory-budget", type=int, default=None, help="Memory budget in MB")
	parser.add_argument("--tracer", type=str, default="default", choices=TRACERS.available())
...
def main():
	...
	sample = model.get_sample(
		mb_size, dtype=dtype, device="cuda"
	)  # sample is needed for autopart AND for ILP ; for simplicity we always set one
	...
	if args.memory_budget is not None and not args.no_optimizer:
		args.memory_budget -= (
			sum(p.nbytes for p in model.parameters()) / world_size / (2**20)
		)  # 2**20 = 1MB
		logger.info(f"Memory budget after subtracting model parameters: {args.memory_budget}MB")
	...
	pipeline = Pipeline(parts, sample, config=pipe_config, signatures=signatures)
	...
	all_params = list(pipeline.parameters())
