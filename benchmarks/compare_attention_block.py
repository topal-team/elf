...
sys.path.append(".")
from models.simple import TransformerBlock
...
if __name__ == "__main__":
	...
	for sdp_backend, dtype in [
		(None, torch.float32),
		(None, torch.float16),
		("FLASH_ATTENTION", torch.float16),
		("FLASH_ATTENTION", torch.bfloat16),
		("MATH", torch.float16),
		("MATH", torch.bfloat16),
	]:
		model = (
			TransformerBlock(
				hidden_dim, num_heads=num_heads, dropout=0.1, ffn_dim=None, sdp_backend=sdp_backend
			)
			.to(device)
			.to(dtype)
		)
