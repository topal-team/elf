...
def main():
    ...
    if rank == 0: print('# of trainable parameters : ', pretty_print_params(sum(p.numel() for p in model.parameters() if p.requires_grad)))
    pipe = Pipeline(model, sample, partition = "metis", schedule = "1f1b")
    loss_fn = lambda x,y, **_: model.loss(x, y)
    ...
    for epoch in range(args.epochs):
        ...
        for batch in tqdm(dataloader, desc=f"Epoch {epoch + 1}/{args.epochs}"):            
            ...
            outputs, loss = pipe(input_ids, input_ids, loss_fn, split_size = args.batch_size // 4)
            # outputs = model(input_ids)
            # loss = model.loss(outputs, input_ids)

            optimizer.step()

            if rank == ws - 1: total_loss += loss.item()
