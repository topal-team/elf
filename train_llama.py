...
def parse_args():
    ...
    parser.add_argument('--train.weight_decay', type=float, default=1e-5, help='Training: weight decay')
    ...
    parser.add_argument("--train.max_seq_len", type=int, default=512, required=False, help="Maximum sequence length")
    ...
    parser.add_argument('--pipeline.pp', type=int, default=4, help='Pipeline: PP degree, number of stages in pipeline parallelism')
    parser.add_argument('--pipeline.dp', type=int, default=1, help='Pipeline: DP degree, number of model copies to process in parallel')
    parser.add_argument('--pipeline.schedule_type', type=str, default="afab", help='Pipeline: default schedule to use', choices=['1f1b', 'afab'])
...
def prepare_tokenizer(args):
    if not os.path.exists(f'{args.tokenizer_dir}'):
        ...
        if not os.path.exists(f'{args.data.__getattr__(f"{args.data.dataset_name}").dataset_dir}/tokenized/train'):
...
def main(args):
    ...
    dataloader = DataLoader(
        ...
        sampler=DistributedSampler(
            tokenized_dataset, 
            num_replicas=args.pipeline.dp, 
            rank=rank // args.pipeline.pp
            ),
    )
