...
def pipelined_backward(layer):
    ...
    for i in range(len(layer.activations)):
        ...
        if global_rank != world_size - 1:
            ...
            (layer.activations[i] * grads).sum().backward() # hack to pass gradients : d(wx)/dx = w, here gradients of activations will be equal to "grads"
...
if __name__ == "__main__":
    ...
    if global_rank == world_size - 1:
        assert result.shape == sample.shape, f'Expected result to have shape {sample.shape}, got {result.shape}'        
        ...
        print(f'Forward pass output is correct.')
    ...
    if global_rank == world_size - 1:
        print(f'Backward pass finished for all processes.')
    ...
    if global_rank == 0:
        ...
        assert grads_full.shape == grads.shape, f'Expected grads to have shape {grads_full.shape}, got {grads.shape}'
