...
sys.path.append(".")
from elf import Pipeline, PipelineConfig
...
if __name__ == "__main__":
	...
	config = PipelineConfig(scheduler="1f1b", partitioner="constrained", placement="auto")

	model = resnet50()
	sample = torch.randn((8, 3, 224, 224)).cuda()
	model = Pipeline(model, sample, config=config)
